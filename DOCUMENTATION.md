...
```bash
git clone https://github.com/jonscafe/HackL4bs.git
cd HackL4bs
...
HackL4bs starts each instance from a per-instance copy of the challenge directory. This keeps dynamic flag injection, resource-limit rewrites, and bind-mounted challenge files stable until the instance is stopped.

HackL4bs also validates compose files before startup and rejects options that would bypass isolation, including `privileged`, `network_mode`, host/container namespaces, added capabilities/devices, Docker socket mounts, external networks/volumes, unsafe build or env-file paths, absolute/home/environment-expanded bind sources, and bind paths containing `..`.
...
- `AUTH_MODE=ctfd`: send `Authorization: Bearer <CTFd access token>`. HackL4bs validates the token via CTFd and requires the CTFd user `type` to be `admin`.
...
Users authenticate with their CTFd access token. HackL4bs accepts the token as a bearer token, validates it against CTFd's API (`/api/v1/users/me`), then fetches the detailed user record from `/api/v1/users/{id}`.
...
The admin dashboard uses the same CTFd access-token flow as the user dashboard, but HackL4bs additionally checks the CTFd user role:

1. The browser sends `Authorization: Bearer <token>` to `/admin/api/me`.
2. HackL4bs calls CTFd `/api/v1/users/me` with that token and reads the authenticated user's `id`.
3. HackL4bs calls CTFd `/api/v1/users/{id}` with the same token.
...
In no-auth mode, admin endpoints require the local `ADMIN_KEY` via `X-Admin-Key`. If HackL4bs is behind a reverse proxy, set `TRUSTED_PROXIES` to the proxy IPs or CIDRs; otherwise forwarded IP headers are ignored to prevent IP spoofing.
...
- In no-auth mode, enter the local `ADMIN_KEY` configured for HackL4bs.
...
### 6. Settings
- ⚙️ **Live Settings** - Update editable HackL4bs settings without restarting the service
...
HackL4bs includes production-ready infrastructure components for reliable, scalable deployments.
...
DATABASE_URL=sqlite+aiosqlite:///./data/HackL4bs.db

# PostgreSQL (for production)
DATABASE_URL=postgresql+asyncpg://user:pass@db:5432/HackL4bs
```

#### 2. Distributed Locking (Redis)

Prevents race conditions when running multiple Gunicorn workers. Spawn checks, persistent port assignment, and Docker Compose startup are protected by locks; with Redis enabled, HackL4bs holds a distributed port-allocation lock until compose startup has bound the selected ports.
...
| `NETWORK_PREFIX` | `HackL4bs` | Prefix for instance networks |
| `ADMIN_RATE_LIMIT` | `150` | Admin API requests allowed per minute per client IP |
...
HackL4bs's production infrastructure adds minimal overhead:

| Component | RAM | CPU | Disk | Notes |
|-----------|-----|-----|------|-------|
| **HackL4bs App** | ~100 MB | 0.1-0.5 cores | - | FastAPI + uvicorn |
...
#### Formula
```
Base Overhead = 200 MB (HackL4bs + Redis + SQLite)
...
HackL4bs enforces global caps (`CONTAINER_MAX_MEMORY`, `CONTAINER_MAX_CPU`, and `CONTAINER_PIDS_LIMIT`) on every service before startup. If a challenge requests a larger limit, the global cap wins.
...
- Check database size: `ls -lh data/HackL4bs.db`
...
1. Navigate to **HackL4bs Logs** → **Instance Forensics** tab
...
> 💡 **Important**: The examples below are **templates/suggestions only**. Challenge authors are **free to use any logging method** they prefer. HackL4bs does not enforce or depend on any specific logging library or format. As long as your application logs to **stdout/stderr**, Instance Forensics will capture it automatically.

#### Freedom of Implementation

You are **not tied to HackL4bs** for logging implementation:
...
- ✅ **No dependency on HackL4bs** - your challenge code remains portable

**The only requirement**: If you want HackL4bs's Instance Forensics to capture your logs, print them to **stdout** or **stderr**. That's it!
...
#### Integration with HackL4bs

Once deployed to HackL4bs:
...
4. View in Admin Dashboard → HackL4bs Logs → Instance Forensics
...
- ✅ Longer retention (beyond HackL4bs's limits)
- ✅ Keep logs even after challenge deletion

**Note**: External services are **your responsibility** - HackL4bs doesn't manage them.
...
**Remember**: HackL4bs doesn't care **how** you log or **what** you log. It simply captures whatever your containers print. The choice is yours!

---

## 🔍 Resource Monitoring

HackL4bs includes native resource monitoring to track CPU and memory usage of containers and the host system. This helps identify resource-intensive instances and prevent server overload.
...
HackL4bs uses color-coded badges for quick identification:
...
3. **Resource Limits** - Set proper `mem_limit` and `cpus` in challenges; HackL4bs also enforces global caps
...
HackL4bs supports **CTFd Team Mode** where instances and dynamic flags are shared per-team instead of per-user. This is essential for team-based CTF competitions.
...
With `TEAM_MODE=auto` (default), HackL4bs automatically detects CTFd's competition mode:
