...
<body>
  <div class="container">
    ...
    <p class="desc">
      Someone encoded their flag with a simple XOR cipher and compiled it into a Linux ELF binary.
      The key is just one byte. The encoded bytes are in the binary. How hard can it be?
    </p>
    <a href="/download" class="download-btn">&#11015; Download Binary (x86-64 ELF)</a>
    <div class="info">
      <h3>Hints</h3>
      <ul style="padding-left:18px; line-height:2;">
        <li>Run it: <code>./challenge &lt;flag&gt;</code></li>
        <li>Tools: <code>gdb</code>, <code>ghidra</code>, <code>radare2</code>, <code>objdump</code></li>
        <li>Set a breakpoint at <code>strcmp</code> to read the decoded string at runtime</li>
      </ul>
    </div>
  </div>
</body>
...
@app.route('/download')
def download():
    return send_file('/app/dist/challenge', as_attachment=True, download_name='challenge')
