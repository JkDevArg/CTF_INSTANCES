...
class DockerClient:
    ...
    def get_host_ports_in_use(self) -> set:
        """
        Return the set of host (external) ports currently bound by ALL
        running Docker containers on this host.

        This is the only reliable way to know which ports Docker has
        reserved — a socket.bind() check from inside the Whaley container
        only tests the container's own network namespace, not the host's.
        """
        used: set = set()
        try:
            containers = self.client.containers.list()
            ...
            print(f"[DockerClient] Warning: could not read host ports in use: {e}")
        return used
