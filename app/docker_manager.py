...
class DockerManager:
    ...
    def is_challenge_active(self, challenge_id: str) -> bool:
        """Check if a challenge is active (supports local or CTFd numeric ID)."""
        # Resolve to local ID first (so the inactive set lookup is always by local ID)
        resolved = self.resolve_challenge_id(challenge_id)
        if resolved is None:
            # Unknown challenge → treat as inactive to be safe
            return False
        return resolved not in self._inactive_challenges
    ...
    def resolve_challenge_id(self, challenge_id: str) -> Optional[str]:
        """
        Resolve a challenge_id to the local Whaley ID.

        Accepts:
        - Local ID (e.g. "pwn-challenge")  → returned as-is if found
        - CTFd numeric ID (e.g. "2") → looked up via flag_manager challenge_mapping (inverted)
        ...
        # Try as CTFd numeric ID via the flag_manager inverse mapping
        try:
            ctfd_id = int(challenge_id)
            flag_mgr = get_flag_manager()
            print(f"[resolve_challenge_id] DEBUG: Looking for {ctfd_id}. Mapping: {flag_mgr.challenge_mapping}")
            print(f"[resolve_challenge_id] DEBUG: Available challenges: {list(self.challenges.keys())}")
            for local_id, mapped_ctfd_id in flag_mgr.challenge_mapping.items():
                try:
                    if int(mapped_ctfd_id) == ctfd_id:
                        if local_id in self.challenges:
                            print(f"[resolve_challenge_id] Mapped CTFd ID {ctfd_id} -> local '{local_id}'")
                            return local_id
                        else:
                            print(f"[resolve_challenge_id] DEBUG: Found mapping to {local_id} but it's NOT in self.challenges!")
                except (ValueError, TypeError):
                    continue
        except (ValueError, TypeError):
            pass

        # Case-insensitive local ID lookup as last resort
        challenge_id_lower = challenge_id.lower()
        for local_id in self.challenges:
            if local_id.lower() == challenge_id_lower:
                return local_id

        return None

    def get_challenge(self, challenge_id: str) -> Optional[ChallengeConfig]:
        """Get a specific challenge configuration (supports local or CTFd numeric ID)."""
        resolved = self.resolve_challenge_id(challenge_id)
        return self.challenges.get(resolved) if resolved else None
    ...
    async def spawn_instance(
        ...
        # Resolve challenge_id: accept local IDs OR CTFd numeric IDs
        resolved_id = self.resolve_challenge_id(challenge_id)
        if not resolved_id:
            return False, f"Challenge not found: {challenge_id}", None
        # Use the canonical local ID everywhere from here on
        challenge_id = resolved_id

        # Get challenge config (guaranteed to exist after resolution)
    ...
    async def _do_spawn_instance(
        ...
        # Start containers — retry up to 3 times on port conflicts
        PORT_CONFLICT_RETRIES = 3
        for attempt in range(1, PORT_CONFLICT_RETRIES + 1):
            try:
                ...
                error_detail = str(e)
                is_port_conflict = "port is already allocated" in error_detail or "address already in use" in error_detail.lower()

                if is_port_conflict and attempt < PORT_CONFLICT_RETRIES:
                    print(f"[PortConflict] Attempt {attempt}/{PORT_CONFLICT_RETRIES}: port conflict detected, retrying with new ports...")

                    # Release the conflicting ports from our tracker
                    self.port_manager.release_instance_ports(instance_id)

                    # Refresh Docker's live port snapshot so we skip them next time
                    try:
                        docker_ports = self.docker.get_host_ports_in_use()
                        self.port_manager._docker_ports_in_use.update(docker_ports)
                        # Also mark them in allocated_ports so allocate_port() skips them
                        for p in docker_ports:
                            if self.port_manager.port_start <= p <= self.port_manager.port_end:
                                self.port_manager.allocated_ports.add(p)
                    except Exception as pe:
                        print(f"[PortConflict] Could not refresh docker ports: {pe}")

                    # Cleanup the failed temp dir (a new one will be created on retry)
                    self._cleanup_instance_temp_dir(instance)

                    # Remove the partially-started network if any
                    if network_name:
                        try:
                    ...
                    # Reallocate ports with refreshed Docker cache
                    new_port_mapping = await self.port_manager.allocate_ports_for_user(
                        instance_id=instance_id,
                        user_id=owner_id,
                        challenge_id=challenge_id,
                        internal_ports=challenge.ports,
                    ...
                    if new_port_mapping is None:
                        instance.status = InstanceStatus.ERROR
                        instance.error_message = "No available ports after conflict retry"
                        del self.instances[instance_id]
                        return False, "No available ports", None

                    # Update instance with the new port mapping
                    instance.ports = new_port_mapping
                    public_host = settings.get_public_host()
                    public_urls: Dict[int, str] = {}
                    for internal_port, external_port in new_port_mapping.items():
                        public_urls[internal_port] = f"{public_host}:{external_port}"
                    instance.public_urls = public_urls
                    if public_urls:
                        first_internal_port = list(new_port_mapping.keys())[0]
                        instance.public_url = public_urls[first_internal_port]

                    # Recreate network for retry
                    if settings.NETWORK_ISOLATION_ENABLED and network_name:
                        try:
                            ...
                            print(f"[PortConflict] Could not recreate network: {ne}")
                            network_name = None
    ...
    def _validate_service_security(service_name: str, service_config: Dict) -> None:
        ...
        for key in ('volumes_from', 'devices', 'device_cgroup_rules'):
            if service_config.get(key):
                raise ValueError(f"Service '{service_name}' cannot use {key}")

        # Whitelist for capabilities (needed for some PWN challenges)
        ALLOWED_CAP_ADD = {'SYS_PTRACE'}
        cap_add = service_config.get('cap_add')
        if cap_add:
            if isinstance(cap_add, str): cap_add = [cap_add]
            for cap in cap_add:
                if cap.upper() not in ALLOWED_CAP_ADD:
                    raise ValueError(f"Service '{service_name}' cannot use capability {cap}")

        # Whitelist for security options
        ALLOWED_SECURITY_OPT = {'no-new-privileges:true', 'seccomp:unconfined', 'seccomp=unconfined'}
        sec_opt = service_config.get('security_opt')
        if sec_opt:
            if isinstance(sec_opt, str): sec_opt = [sec_opt]
            for opt in sec_opt:
                if opt.lower() not in ALLOWED_SECURITY_OPT:
                    raise ValueError(f"Service '{service_name}' cannot use security_opt {opt}")
    ...
    async def stop_instance(
        ...
        # Only the user who created the instance can stop it (even in team mode)
        if user_id and instance.user_id != user_id:
            return False, "Solo quien inició la instancia puede detenerla"
        if team_mode and team_id and instance.team_id != team_id:
        ...
        try:
            ...
            # Clean up the flag if not solved
            from app.flag_manager import get_flag_manager
            flag_mgr = get_flag_manager()
            await flag_mgr.cleanup_instance_flag(
                local_challenge_id=instance.challenge_id,
    ...
    async def start_cleanup_task(self) -> None:
        ...
        async def cleanup_loop():
            cleanup_counter = 0
            port_refresh_counter = 0
            while True:
                await asyncio.sleep(60)  # Check every minute
                await self.cleanup_expired()

                # Refresh Docker host port cache every 5 minutes
                # (keeps PortManager in sync if external containers grab ports)
                port_refresh_counter += 1
                if port_refresh_counter >= 5:
                    port_refresh_counter = 0
                    try:
                        docker_ports = self.docker.get_host_ports_in_use()
                        self.port_manager._docker_ports_in_use = docker_ports
                        # Sync into allocated_ports for our range
                        for p in docker_ports:
                            if self.port_manager.port_start <= p <= self.port_manager.port_end:
                                self.port_manager.allocated_ports.add(p)
                        print(f"[PortManager] Refreshed Docker port cache: {len(docker_ports)} host ports in use")
                    except Exception as e:
                        print(f"[PortManager] Port cache refresh failed: {e}")
...
def get_docker_manager(port_manager: Optional[PortManager] = None) -> DockerManager:
    """Get the global docker manager instance."""
    global _docker_manager
    if _docker_manager is None:
        if port_manager is None:
            raise ValueError("DockerManager not initialized and no PortManager provided")
        _docker_manager = DockerManager(port_manager)
