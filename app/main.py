...
docker_manager = get_docker_manager(port_manager)
...
@app.post("/admin/api/check-flag")
async def admin_check_flag(
    request: Request,
    _: bool = Depends(verify_admin_key)
):
    """
    Check who owns a submitted flag string.
    Returns ownership info so CTFd can enforce team-sharing penalties in real time.
    Body: {"flag": "FLAG{...}"}
    """
    body = await request.json()
    flag_content = (body.get("flag") or "").strip()
    if not flag_content:
        raise HTTPException(status_code=400, detail="flag field required")

    flag_mgr = get_flag_manager()
    flag_id = flag_mgr.flag_lookup.get(flag_content)

    if flag_id is None:
        return {"found": False}

    mapping = flag_mgr.flag_mappings.get(flag_id)
    if not mapping:
        return {"found": False}
    ...
    return {
        ...
        "owner_team_id": mapping.team_id,
        "owner_username": mapping.username,
        "owner_team_name": mapping.team_name,
        "local_challenge_id": mapping.local_challenge_id,
        "ctfd_challenge_id": mapping.ctfd_challenge_id,
    }
...
# =============================================================================
# HackL4bs Settings API (Global Settings via UI)
# =============================================================================
...
async def admin_get_settings(_: bool = Depends(verify_admin_key)):
    """Get all editable HackL4bs settings with current values."""
    from .database.connection import get_async_session
    from .database.models import WhaleySettings as WhaleySettingsModel
    from sqlalchemy import select
...
async def admin_update_settings(
    ...
    """Update HackL4bs settings. Changes are persisted to database and applied immediately."""
    from .database.connection import get_async_session
    from .database.models import WhaleySettings as WhaleySettingsModel
    from sqlalchemy import select
