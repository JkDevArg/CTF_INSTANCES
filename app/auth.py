...
class CTFdAuth:
    ...
    async def validate_token(self, token: str) -> Optional[UserInfo]:
        ...
        try:
            ...
            print(f"CTFd auth error: Could not connect to CTFd at {self.ctfd_url}. Check if the service is running and accessible.")
            return None
        except Exception as e:
            print(f"CTFd auth error: {type(e).__name__}: {e}")
            return None
