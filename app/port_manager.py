...
class PortManager:
    ...
    def __init__(self, port_start: int, port_end: int):
        ...
        # Cache of Docker host ports — refreshed at startup and on demand
        self._docker_ports_in_use: Set[int] = set()
    
    async def initialize(self) -> None:
        """Initialize port manager and seed already-allocated ports from Docker."""
        if self._initialized:
            return

        # Query Docker for ports already bound on the host so we never
        # double-allocate a port that another container already owns.
        try:
            from .docker_client import get_docker_client
            docker = get_docker_client()
            self._docker_ports_in_use = docker.get_host_ports_in_use()
            # Pre-seed allocated_ports with Docker's live bindings in our range
            for port in self._docker_ports_in_use:
                if self.port_start <= port <= self.port_end:
                    self.allocated_ports.add(port)
            print(f"[PortManager] Seeded {len(self._docker_ports_in_use)} ports from Docker host bindings ({len(self.allocated_ports)} in our range)")
        except Exception as e:
            print(f"[PortManager] Warning: could not seed ports from Docker: {e}")
            self._docker_ports_in_use = set()
    ...
    def _is_port_available(self, port: int) -> bool:
        ...
        Checks both:
        1. Our internal allocated_ports set (which includes Docker-seeded ports)
        2. A TCP bind on 0.0.0.0 as a secondary check (best-effort from container)
        """
        if port in self.allocated_ports:
            return False

        # Also check live Docker bindings cache (may have grown since init)
        if port in self._docker_ports_in_use:
            self.allocated_ports.add(port)  # keep set in sync
            return False

        # Secondary: try a local bind (catches OS-level reservations visible
        # in this network namespace — e.g. ephemeral ports, TIME_WAIT sockets)
        try:
